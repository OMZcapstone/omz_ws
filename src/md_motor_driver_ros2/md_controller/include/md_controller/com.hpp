...
typedef struct {
    ...
    int rpm,position;
    int rpm_by_id[3];
    int position_by_id[3];
    int feedback_seen_by_id[3];
    int pnt_rpm[2];
    int pnt_position[2];
    int pnt_feedback_seen;
    int last_rcv_id;
    int last_rcv_pid;
    int last_rcv_data_size;
    int last_main_data[18];
...
typedef struct{
    int ID, GearRatio, InitError, poles, OdomEncoderPPR;
    BYTE InitMotor;
    short rpm;
    long position;
...
extern void SetSerialDebug(bool enabled);
